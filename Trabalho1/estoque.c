...
Medicamento *CriaMedicamento(char *nome, int codigo, float valor, int *data_de_validade) {
    ...
    // Atribuindo os valores para os campos c�digo e valor
    novo->codigo = codigo;
    novo->valor = valor;
...
Lista *InsereListaMedicamento(FILE *fp, Lista *l, Medicamento *n) {
    // Cria��o de um ponteiro para a struct lista para fazer a atribui��o na lista
    Lista *lst = (Lista *)malloc(sizeof(Lista));
    // Ponteiro lista apontando para a vari�vel do tipo medicamento dentro da struct lista e atribuindo a lista recebida como par�metro
    lst->m = n;
    // Setando o pr�ximo elemento a ser preenchido
    lst->prox = l;
...
Lista *RetiraListaMedicamento(FILE *fp, Lista *l, int id_medicamento) {
    // Vari�vel para armazenar o ponteiro anterior na lista
    Lista *ant = NULL;
    // Vari�vel para percorrer a lista
    Lista *p = l;

    fprintf(fp, "\n");
    // Percorrendo a lista at� encontrar o medicamento com o c�digo especificado
    while (p != NULL && p->m->codigo != id_medicamento) {
        ant = p;
        p = p->prox;
    }

    // Se n�o encontrou o medicamento
    if (p == NULL) {
        fprintf(fp, "CODIGO DO MEDICAMENTO INVALIDO OU NAO ENCONTRADO\n");
        return l;
    }

    // Se o medicamento a ser removido � o primeiro da lista
    if (ant == NULL) {
        l = p->prox;
    } else {
        // Se o medicamento a ser removido n�o � o primeiro
        ant->prox = p->prox;
    }

    fprintf(fp, "MEDICAMENTO %s REMOVIDO\n", p->m->nome);
    // Liberando a mem�ria do medicamento e do n� da lista
    free(p->m);
    free(p);
    return l;
}
...
int VerificaListaMedicamento(FILE *fp, Lista *p, int id_medicamento) {
    ...
    // Percorrendo a lista para verificar se o medicamento est� presente
    while (aux != NULL) {
...
void Amnesia(Lista *l) {
    Lista *p = l;
    // Percorrendo a lista e liberando a mem�ria de cada n�
    while (p != NULL) {
        Lista *t = p->prox;
        free(p->m);
        free(p);
